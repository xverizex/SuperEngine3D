...
{
	if (argc < 3) { printf ("error: no select file\n"); return -1; }
	...
	char *new_name = get_name (argv[3]);

	int i = 1;
	FILE *fo = fopen (new_name, "w");
	...
	for (int mm = 1; mm <= count; mm++) {
		...
		char filename[255];
		snprintf(filename, 255, "%s%d.obj", argv[1], mm);
		...
		fwrite (&f_size, sizeof (int), 1, fo);
		fwrite (f, sizeof (float) * 24, f_size, fo);
